...
async function openApp(application, parent) {
  ...
  if (!context) {
    ...
    context = { session: isolated, application };
    isolated.setPermissionRequestHandler((_contents, _permission, callback) => callback(false));
    isolated.setPermissionCheckHandler(() => false);
    isolated.webRequest.onBeforeRequest((details, callback) => {
      const url = new URL(details.url);
      callback({ cancel: !((url.protocol === 'aether-app:' && url.hostname === context.application.slug) || ['data:', 'blob:'].includes(url.protocol)) });
    });
    isolated.protocol.handle('aether-app', async request => {
      try {
        const url = new URL(request.url);
        if (url.hostname !== context.application.slug || !['GET','HEAD'].includes(request.method)) return new Response('Forbidden', { status:403 });
  ...
  const window = new BrowserWindow({ width:1400, height:950, minWidth:800, minHeight:600, frame:false, backgroundColor:'#171d2b', webPreferences:{ contextIsolation:true, nodeIntegration:false, sandbox:true, preload:path.join(__dirname,'preload.js') } });
  windows.set(application.id, window);
  const view = new WebContentsView({ webPreferences:{ session:context.session, contextIsolation:true, nodeIntegration:false, sandbox:true } });
  ...
  window.on('closed', () => { windows.delete(application.id); if (!view.webContents.isDestroyed()) view.webContents.close(); if (!parent.isDestroyed()) { parent.show(); parent.focus(); } });
  for (const contents of [window.webContents, view.webContents]) contents.setWindowOpenHandler(() => ({action:'deny'}));
  window.webContents.on('will-navigate', event => event.preventDefault());
  view.webContents.on('will-navigate', (event, url) => { if (!url.startsWith(`aether-app://${application.slug}/`)) event.preventDefault(); });
  resize();
  try {
    await window.loadFile(path.join(__dirname,'index.html'), {query:{name:application.name}});
    await view.webContents.loadURL(`aether-app://${application.slug}/${application.entry.split('/').map(encodeURIComponent).join('/')}`);
